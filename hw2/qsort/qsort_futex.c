...
again:
    ...
    while (qs->st == ts_idle)
        ...
        cond_wait(&qs->fcond_st, &qs->fmutex_st);
    // verify(pthread_mutex_unlock(&qs->mtx_st));
    mutex_unlock(&qs->fmutex_st);
    if (qs->st == ts_term) {
        return NULL;
